...
Monte Carlo (MC) methods are ``a broad class of computational algorithms that rely on  `r myred("repeated random sampling")` to obtain numerical results.'' - Wikipedia 
...
`r sk2()`
We can also use the draws to calculate the posterior distribution of `r myblue("any function of our estimated parameters")`. As the number of draws/samples gets large we can approximate these quantities arbitrarily high precision.
...
\begin{center}
 `r myblue("A summary statistic or other feature of a distribution   (e.g. expected value) can be approximated by the same summary/feature of an _empirical sample_ from that distribution (e.g., sample mean).")` 
\end{center}
...
\begin{center}
\includegraphics[scale=0.275,trim=30 80 0 70]{MCMC}
\end{center}
...
`r sk2()`
Gibbs sampling leverages the _conditional_ distributions of parameters to generate samples by proposing them one at a time. This is the algorithm implemented in the popular Bayesian packages BUGS, WinBUGS, and JAGS/${\tt rjags}$. 
