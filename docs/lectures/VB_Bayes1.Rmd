...
1. estimators that work for small and large sample sizes
1. parsimonious descriptions of data, predictions for missing data, and forecasts for future data
1. a coherent computational framework for model estimation, selection and validation
...
1. Provides more intuitive answers in terms of the probability that parameters have particular values. 
1. In many complicated statistical problems there are no obvious non-Bayesian inference methods.

`r sk1()`

___`r myred("Cons")`___

1.  It can be hard to mathematically formulate prior beliefs (choice of $f(\theta)$ often ad hoc or for computational reasons).
1. Posterior distributions can be sensitive to prior choice.
1. Analyses can be computationally costly.
...
1. Research question
1. Data collection
1. Model $Y_i \approx f(X_i)$ 
1. Estimate the parameter in the model with uncertainty
1. Make inference
 
 `r sk2()`

The difference between `r myblue("Classical")` and `r myblue("Bayesian")` lies in step 4: (C) uses maximum likelihood estimate (MLE), and (B) derives a posterior distribution.
...
plot(x, dbeta(x, 2, 20), type="l", lwd=2, xlab="", ylab="density", cex.lab=0.75, cex.axis=0.75, mgp=c(2,1,0))
```
...
$$
This has the correct _`r myblue("asymptotic")`_ coverage, but __$y=0$__ `r myred("is still problematic")`!

## Conjugate Bayesian Models

Some sets of priors/likelihoods/posteriors exhibit a special relationship called __`r myblue("conjugacy")`__. This happens if the posterior distribution has the same form as the prior. For instance, in our above Beta-Binomial/Bernoilli examples:
\begin{align*}
\theta & \sim  \mathrm{Beta} (a,b) 
Y|\theta &  \sim  \mathrm{Bin} (n, \theta) 
...
__Are all posteriors in the same family as the priors?__ ___`r myred("No")`___ 
...
$\dagger$ a.k.a. the _maximum a posteriori estimator_ (MAP)
...
`r sk2()`
Such intervals are typically called `r myblue("credible intervals")`, to distinguish them from frequentist confidence intervals. Both are referred to as CIs. 
...
`r sk2()`
Using a uniform prior distribution (a.k.a., $\mathrm{Beta}(1,1)$) for $\theta$, the posterior distribution is $\theta | Y=2 ~ \mathrm{Beta}(1+2,1+10-2)$.  
...
where $\theta$ is the success rate of the treatment. Write down the likelihood for the $n$ patients. Then, assuming a $\mathrm{Beta}(a,b)$ prior for $\theta$, find the posterior distribution for $\theta|Y$. Does this look familiar?



##  Next Steps

Next you'll complete a practical where you conduct a conjugate Bayesian analysis for the mean of a normal distribution on the midge data introduced in the likelihood chapter. You'll also visualize the effect of different prior choices on the posterior distribution. You'll also use the results of this when you start trying out MCMC (with JAGS) to approximate posterior distributions numerically
