---
title: An R Markdown document converted from "../Jupyter-notebooks/notebooks/2-Linear_models-NEW_EXAMPLES.ipynb"
...
```{r}
wings <- read.csv(traitdata_Huxleyetal_2021.csv)
```
...
Before we do that, there is a lot of data in the data frame that we don't need for our analysis and we should make sure that we are using the same data for our plots and models. We will subset the data down to the complete data for the three variables:
